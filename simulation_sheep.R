...
if (runSim == 1) {
  ...
  herdSize = simcmp(n = (10 * nHerds),
                    v = c(round(meanHerdSize/10), round(sdHerdSize/10))) # TODO: https://github.com/SimonaAntonios/sheep_simulation_AlphaSimR/issues/1
  # hist(herdSize)
...
if (runSim == 1 | runSim == 2) {
  ...
  for (year in yearToDo:nPTyrs) {
    ...
    if (all(wtRams2@father == "0")) {
      siresOfFemales1 = selectInd(pop = wtRams2[!wtRams2@iid %in% eliteSires1@iid], nInd = nsiresOfFemales1, # siresOfFemales1 are 3.5 years old here
                                  use = use, trait = 1)
    } else {
      siresOfFemales1 = selectWithinFam(pop = wtRams2[!wtRams2@iid %in% eliteSires1@iid], nInd = 2, # siresOfFemales1 are 3.5 years old here
                                        use = use, trait = 1,
                                        famType = "M")
    ...
    # TODO: better to use selectTop (TURE or FALSE) to choose the animals in selectInd ???? 
    ...
    wtRams2 = wtRams1
    wtRams1 = selectWithinFam(pop = lambs[sel], nInd = n,
                              use = use, trait = 1,
    ...
    # TODO: here I need to remove the lambs that are not selected as wtRams1 and ntlMatingRams
    SP$pedigree = SP$pedigree[-c(!lambs@id %in% wtRams1@id, !lambs@id %in% ntlMatingRams@id),]
